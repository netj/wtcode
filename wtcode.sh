...
Usage: wtcode [BRANCH] [CMD [CMD-ARGS...]]
  ...
  CMD        Command to launch in the worktree (default: \$WTCODE_CMD,
             or first available of: claude, aider, codex, \$SHELL).
...
[[ -e "$worktree_path"/.git ]] ||
  if [[ -n ${remote_branch-} ]] && ! git rev-parse --verify "refs/heads/$branch_name" &>/dev/null; then
...
if [[ $# -gt 0 ]]; then
  ...
  IFS=' ' read -ra wtcode_cmd <<< "${WTCODE_CMD:-$(
    for cmd in claude aider codex; do
      type "$cmd" &>/dev/null && echo "$cmd" && exit
    done
    echo "${SHELL:-bash}"
  )}"
fi
...
# tool-specific setup
case "${wtcode_cmd[0]##*/}" in
  claude)
    # auto-trust the worktree in claude's config
